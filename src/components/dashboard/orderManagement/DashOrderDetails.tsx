...
const DashOrderDetails: FC<DashOrderDetailsProps> = ({ manual, orderDetails, setAnchorEl }) => {
  ...
  let purchase_order;
  if (manual) {
    purchase_order = orderDetails?.custom_purchase_order;
  } else {
    purchase_order = orderDetails?.purchase_order;
  }
            ...
            <div>
              <div className="space-y-2 mt-3">
                ...
                <p className="flex items-center gap-2">
                  Total:{' '}
                  <span className="text-sm font-medium xl:text-[16px]">
                    ৳ {getOrderSubtotal(purchase_order)}
                  </span>
                </p>
              </div>
            </div>
                    ...
                    <div className="flex items-center justify-between" key={Math.random()}>
                      <div className="relative flex items-center gap-3">
                        ...
                        <p className="text-sm font-medium flex items-center gap-4">
                          {i?.product?.title}
                          {`(${i?.product?.slug})`}-
                          {i?.others_info?.product_size_varient?.size_title}
